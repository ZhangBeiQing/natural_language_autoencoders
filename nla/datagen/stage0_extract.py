...
def main() -> None:
    ...
    print(
        f"[stage0] start corpus={args.corpus}/{args.corpus_config or '-'} "
        f"split={args.corpus_split} slice=[{args.corpus_start}, "
        f"{args.corpus_start + args.corpus_length}) model={args.base_model} "
        f"layer={args.layer_index} output={args.output}",
        flush=True,
    )
    ...
    print(f"[stage0] loading extractor/model: {args.extractor_cls} kwargs={user_kwargs}", flush=True)
    ...
    print(f"[stage0] extractor ready d_model={d_model}", flush=True)
    ...
    if corpus_path.is_file():
        print(f"[stage0] loading local parquet corpus={corpus_path}", flush=True)
        table = pq.read_table(corpus_path, columns=[args.text_column])
        end = args.corpus_start + args.corpus_length
        assert end <= table.num_rows, (
            f"requested local corpus rows [{args.corpus_start}, {end}) but "
            f"{corpus_path} only has {table.num_rows} rows"
        )
        texts_all = table.column(args.text_column).to_pylist()[args.corpus_start:end]
    else:
        split_expr = f"{args.corpus_split}[{args.corpus_start}:{args.corpus_start + args.corpus_length}]"
        print(f"[stage0] loading dataset slice split={split_expr!r}", flush=True)
        ...
        texts_all = ds[args.text_column]
    print(f"[stage0] dataset ready selected_docs={len(texts_all)}", flush=True)
    ...
    with pq.ParquetWriter(storage.open_write(args.output), schema) as writer:
        for chunk_start in tqdm(range(0, len(texts_all), args.chunk_size), desc="chunks"):
            chunk_end = min(chunk_start + args.chunk_size, len(texts_all))
            print(f"[stage0] extracting docs [{chunk_start}, {chunk_end})", flush=True)
            texts = texts_all[chunk_start:chunk_end]
            ...
            print(
                f"[stage0] wrote chunk docs [{chunk_start}, {chunk_end}) "
                f"rows={len(rows['doc_id'])} total_rows={row_count}",
                flush=True,
            )
